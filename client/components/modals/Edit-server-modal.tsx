...
import { RequestCreateServer, RequestEditServer } from "@/API";
...
const EditServerModal = (props: IEditServerModalProps) => {
    ...
    useEffect(() => {
        if (data) {
            form.setValue("name", data.serverName || "");
    ...
    const handleOnsubmitForm = async (values: z.infer<typeof ServerSchema>) => {
        const res = await RequestEditServer(
            {
